...
def train(
    ...
    typer.secho("Pelatihan selesai. Model terbaik ada di folder 'runs/'.", fg=typer.colors.GREEN)
...
def infer(
    ...
    if not Path(model_path).exists():
        typer.secho(f"Error: File model {model_path} tidak ditemukan.", fg=typer.colors.RED)
        raise typer.Exit(1)
